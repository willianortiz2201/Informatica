#  Informática - Logística (1º Semestre) | FATEC PROF. JESSEN VIDAL

Desenvolvimento prático em análise de dados, ferramentas digitais e lógica de programação

---

## Visão Geral

Repositório com atividades desenvolvidas ao longo do semestre, utilizando ferramentas como Excel, Canva e Python, com foco em organização, análise de dados e lógica.

---

## Conteúdo

### Aulas

* Aula 01 — [Apresentação com Canva](./aula-01-apresentacao-canva)
* Aula 02 — [Tabela no Excel](./aula-02-tabela-excel)
* Aula 03 — [Gráfico no Excel](./aula-03-grafico-excel)
* Aula 04 — [Tabela e Gráfico Dinâmico](./aula-04-tabela-dinamica)
* Aula 05 — [Gráfico Dinâmico](./aula-05-grafico-dinamico)
* Aula 06 — [Tratamento de Dados 1](./aula-06-tratamento-de-dados-1)
* Aula 07 — [Tratamento de Dados 2](./aula-07-tratamento-de-dados-2)

---

### Cursos Complementares

* Curso — [Python Fundamental 1](./python-fundamental-1)

---

## Competências Desenvolvidas

* Organização e análise de dados
* Criação de gráficos e visualizações
* Tratamento de dados com Excel
* Lógica de programação com Python

---

## Tecnologias Utilizadas

<p>
  <img src="https://img.shields.io/badge/Excel-217346?logo=microsoft-excel&logoColor=white" />
  <img src="https://img.shields.io/badge/Canva-00C4CC?logo=canva&logoColor=white" />
  <img src="https://img.shields.io/badge/Python-3776AB?logo=python&logoColor=white" />
</p>

---

## Estrutura

Cada atividade contém:

* descrição do que foi realizado
* aplicação prática
* resultado visual

---

